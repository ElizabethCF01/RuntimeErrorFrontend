...
<template>
  ...
  <div class = 'demo-control-panel flex-row row'>
    <div class = 'col-6 mb-2'>
      <label>Node:</label>
      <button @click = 'addNode' class='btn btn-primary btn-sm mx-2'>add</button>
      <button class='btn btn-danger btn-sm' :disabled = 'selectedNodes.length == 0' @click = 'removeNode'
        >remove</button>
    </div>
    <div class = 'col-6 mb-2'>
      <label>Edge:</label>
      <button class='btn btn-primary btn-sm mx-2' :disabled = 'selectedNodes.length != 2' @click = 'addEdge'
        >add</button>
      <button class='btn btn-danger btn-sm' :disabled = 'selectedEdges.length == 0' @click = 'removeEdge'
        >remove</button>
    </div>
  </div>
    <v-network-graph
  ...
  <div class='mt-5'>
      <label>BFS:</label>
      <input class='mx-2' v-model="ini"/>
      <button class='btn btn-primary btn-sm mx-2' :disabled = 'ini == ""' @click = 'bfs = graph.bfs(ini)'
      >try</button>
      <div class = 'd-flex flex-row justify-content-center mt-2'>
        <div v-if="bfs == -1">El nodo no existe</div>
        <div v-else v-for="(x, i) in bfs.split(' ')" :key="i" class="mx-1">
          <span v-if="x!=''" class="badge bg-primary">{{x}}</span>
        </div>
    </div>
  </div>
  </div>
